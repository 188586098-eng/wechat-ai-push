...
function parseInfoq(html) {
  ...
  const root = resolve(3);
  const homeBase = root && (root.homeBase || root['home-base']);
  const day = (homeBase && homeBase.hotList && homeBase.hotList.day) || [];

  return day
    .filter((it) => it && (it.article_title || it.title) && it.aid)
    .map((it) => ({
      title: cleanTitle(it.article_title || it.title),
      url: `https://www.infoq.cn/article/${it.aid}`,
      pubTime: '',
    }));
}
