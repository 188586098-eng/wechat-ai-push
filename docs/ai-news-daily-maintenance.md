...
### 2026-09-02 守卫 24h 滑动窗口跨日误判 → 改为北京自然日窗口

- **现象**：9/2 推送延迟至北京时间 12:47（兜底槽本应在 08:20 推送）
- **根因（8/30 修复仍带缺陷）**：去重守卫用"滑动 24h 窗口"判断今日是否已推送。主槽延迟补跑常发生在次日凌晨/上午（8/31 推于 08:41、9/1 推于 09:19），当日兜底槽与"昨日延迟的真实推送"相隔不足 24h，被误判"今天已推"而跳过（run#33）；随后延迟的主槽补跑（run#34）因 24h 窗口已过、兜底又无 artifact，判定未推而在 12:47 推送
- **修复**：窗口从"滑动 24h"改为"**北京时间自然日 0 点起**"（北京=UTC+8 折算）——只有北京时间今天真实推送过（success + artifact）才算已推；昨日任何时段的推送均不拦截今日槽位
- **验证**：8 个单测全过（含北京自然日窗口起点断言、跨日不拦截、死锁防护、fail-open）
- 对应 commit：`fc67574`（修复）、`6ca1290`（README）
- **教训（续）**：守卫窗口的时间语义必须与业务"天"对齐（北京时间自然日），滑动时长窗口在跨日延迟场景下必然误判

### 2026-08-30 守卫死锁导致连续断推 → 改用 artifact 判定

- **现象**：8/29、8/30 连续两天微信未收到日报（最后一次真实推送为 8/28 14:27）
- **根因（8/28 方案自引入的缺陷）**：去重守卫把"近 24h 内有其他 success 的 schedule 运行"当作"已推送"，但**被守卫跳过的运行结论也是 success**——"跳过"被当"已推送"链式传递（run#24 因 #22 跳过、#25 因 #24 跳过……），永不推送的死锁
- **修复**：判定改为"其他 schedule 运行 **success 且带日报 artifact**"。只有真实推送的运行才产出 `ai-news-daily-report` artifact（实测：真实推送 artifacts=1，跳过的=0），跳过运行无 artifact，死锁自动解除；fail-open 保留
- **验证**：6 个单测全过（含死锁核心场景）；dispatch run#28 补推真实成功（58 条，artifact 1 个）
- 对应 commit：`17559bf`（修复）、`ab2a2bb`（README）
- **教训**：**用"运行结论"代理"业务结果"不可靠**——守卫/监控的判定依据必须是业务的直接产物（artifact、输出文件），而非执行状态（success/failure）
...
- **（8/30 补充）去重判定必须用业务产物**：光看运行 success 会把"被守卫跳过的运行"当"已推送"，链式死锁永不推送；正确做法是判定"成功运行是否带日报 artifact"
- **（9/2 补充）守卫窗口要用北京自然日而非滑动 24h**：主槽延迟补跑可落在次日凌晨/上午，滑动窗口会把昨日推送误当今日已推，导致今日兜底被跳、推送拖到下午；窗口起点 = 北京今日 0 点折算的 UTC 时刻
- 守卫查询 runs API 需 workflow 显式声明 `permissions: actions: read`，token 用 `${{ github.token }}`

## 当前状态（2026-09-02 确认）

- 定时推送：每天北京时间 **06:23 主槽位 + 08:23 兜底槽位**，脚本内去重（北京自然日窗口 + artifact 判定）保证只推一次
- 重试保护：已上线生效（推送接口网络层异常自动重试）
- 失败通知：已上线（job 失败时经 PushPlus 推送提醒，含日志链接）
- 去重守卫：已上线；经历过两次缺陷修复——(1) 判定改用 artifact 破除跳过死锁；(2) 窗口改为北京自然日破除跨日误判
