...
### 2026-09-14 运行健康体检 + 主槽位按实测延迟补偿

- **体检结论**：近 12 天（9/3–9/14）健康，每天恰好 1 次真实推送 + 1 次兜底槽被守卫正确跳过，**0 漏推 / 0 重复 / 0 失败**；日报 73–81 条，无 LLM 增强回退
- **累计统计（8/9 起 58 次运行）**：真实推送 38 次、跳过 20 次、失败 1 次（8/20）
- **关键发现**：主槽触发后的 GitHub 调度延迟**稳定在 1h39m–2h09m**（12 天全部如此，非偶发随机）——原先设北京 06:23 的主槽实际总在 08:02–08:32 送达
- **调整**：主槽 cron `23 22 * * *`（北京 06:23）→ `7 21 * * *`（北京 05:07），按约 2h 延迟做前向补偿，预期 **06:47–07:16 送达**；分钟偏移 7 分避整点高峰
- **兜底槽保持 `23 0 * * *`（北京 08:23）不变**：与主槽间隔 3h16m，确保两槽实际执行时间不重叠——若间隔过小（如 2h），两槽可能在延迟波动下同时执行、双双判定"今日未推"而重复推送
- **证伪的怀疑**：Google AI Blog / DeepMind Blog 日志常显示 0 条，一度怀疑 RSS 失效；实测两个源均有效（可正常拉取，最新文章 9/10、9/8），仅因更新频率低（每周 1–3 篇），24h 窗口内常无新文。日志零 `[跳过]` 报错亦印证抓取正常
- 对应 commit：`a68b6a4`（cron）、`071660a`（README）
...
- **（9/14 补充）调度延迟可能是系统性的，可实测后前向补偿**：本仓库主槽延迟稳定在 1h39m–2h09m（连续 12 天），并非随机抖动；据此把 cron 提前约 2 小时即可把送达时间校准到目标点
- **（9/14 补充）双槽位间隔必须大于延迟波动范围**：若两槽间隔过小（如 2h），延迟波动可能让两槽几乎同时执行，双双判定未推送而重复发；本仓库保持 3h16m 间隔
- **（9/14 补充）来源 0 条 ≠ 来源失效**：先看日志有无 `[跳过]` 抓取报错；无报错说明拉取成功，0 条只是时间窗口内无新文。低产源（更新频率每周 1–3 篇）在 24h 窗口内常态为 0

## 当前状态（2026-09-14 确认）

- 定时推送：每天北京时间 **05:07 主槽位 + 08:23 兜底槽位**；按实测约 2h 调度延迟补偿，预期 **06:47–07:16 送达**
- 实测调度延迟：主槽 1h39m–2h09m（连续 12 天稳定），兜底槽一轮约 4h+
- 近 12 天健康度：每天恰好 1 次真实推送 + 1 次兜底槽被正确跳过，0 漏推 / 0 重复 / 0 失败；日报 73–81 条
